...
(function() {
  var CND, alert, badge, debug, dec, decG, echo, help, hex, hexG, info, log, name, nameO, nameOG, rpr, urge, warn, whisper;
  ...
  this.unicode_isl = (function(_this) {
    return function() {
      var R, i, interval, len, ref;
      R = _this._ISL["new"]();
      _this._ISL.add_index(R, 'rsg');
      _this._ISL.add_index(R, 'tag');
      ref = require('../data/unicode-9.0.0-intervals.json');
      for (i = 0, len = ref.length; i < len; i++) {
        interval = ref[i];
        _this._ISL.add(R, interval);
      }
      _this._aggregate = _this._ISL.aggregate.use(R);
      return R;
    };
  })(this)();

  this._copy_library = function(input_default) {
    var R, mix, reducers;
    ...
    reducers = {
      fallback: 'assign',
      fields: {
        unicode_isl: (function(_this) {
          return function(values) {
            return _this._ISL.copy(_this.unicode_isl);
          };
        })(this)
      }
    };
  ...
  this._XXX_escape_chrs = (function(_this) {
    return function(text) {
      return _this.cloak.backslashed.hide(_this.cloak.hide(text));
    };
  })(this);

  this._XXX_unescape_escape_chrs = (function(_this) {
    return function(text) {
      return _this.cloak.reveal(_this.cloak.backslashed.reveal(text));
    };
  })(this);

  this._XXX_remove_escaping_backslashes = (function(_this) {
    return function(text) {
  ...
  this.chrs_from_text = function(text, settings) {
    var idx, input_mode, is_escaped, last_idx, parts, raw_part, raw_parts, ref, ref1, splitter;
    if (text.length === 0) {
      return [];
    }
    ...
    while (true) {
      ...
      if (is_escaped = !is_escaped) {
        if (raw_part === '\\') {
          parts[parts.length - 1] = parts[parts.length - 1] + '\\' + ((ref = raw_parts[idx + 1]) != null ? ref : '');
          idx += 2;
          continue;
        }
        if (idx % 2 === 0) {
    ...
    debug('77621', parts);
    switch (input_mode = (ref1 = settings != null ? settings['input'] : void 0) != null ? ref1 : this._input_default) {
      case 'plain':
  ...
  this._csg_cid_from_hint = function(cid_hint, settings) {
    ...
    * Before CSG and CID are returned, they will be validated for plausibility.
     */
    var cid, csg, csg_of_cid_hint, csg_of_options, input_mode, ref, type;
    ...
    switch (type = CND.type_of(cid_hint)) {
      case 'number':
        ...
        break;
      case 'text':
        ref = this.csg_cid_from_chr(cid_hint, {
          input: input_mode
        }), csg_of_cid_hint = ref[0], cid = ref[1];
        break;
      default:
