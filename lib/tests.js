...
(function() {
  ...
  this._Unicode_demo_compile_base_intervals = function(intervals) {
  ...
  this._Unicode_demo_compile_planes = function(intervals) {
  ...
  this._Unicode_demo_compile_areas = function(intervals) {
    var hi, i, len, lo, name, ref, ref1, rsg_registry, short_name, type;
    rsg_registry = require('./character-sets-and-ranges');
  ...
  this._Unicode_demo_compile_blocks = function(intervals) {
  ...
  this._Unicode_demo_read_or_write_cache = function() {
    ...
    if (FS.existsSync(cache_route)) {
      ...
      intervals = [];
      this._Unicode_demo_compile_base_intervals(intervals);
      this._Unicode_demo_compile_planes(intervals);
      this._Unicode_demo_compile_areas(intervals);
      this._Unicode_demo_compile_blocks(intervals);
  ...
  this["Unicode demo"] = function(T) {
    var ISL, cid, glyph, i, j, len, reducers, ref, u;
    ISL = require('interskiplist');

    /* General data */
    u = this._Unicode_demo_read_or_write_cache();
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      glyph = ref[i];
      urge(glyph, ISL.aggregate(u, glyph, {
        reducers: reducers
      }));
    }
    for (cid = j = 0x9fd5; j <= 40918; cid = ++j) {
